...
module "RG_AADSGs" {
  ...
  azuread_groupsMap = {
    Contributors = {
      userDefinedString = "${var.group}_${var.project}_${local.resource_groups_L2.IT_KB.name}_Contributors"
      owners            = var.L2_RG_RBAC.contributorNames
    },
    Owners = {
      userDefinedString = "${var.group}_${var.project}_${local.resource_groups_L2.IT_KB.name}_Owners"
...
resource "azurerm_role_assignment" "L2_RG_Owner" {
  scope                = local.resource_groups_L2.IT_KB.id
