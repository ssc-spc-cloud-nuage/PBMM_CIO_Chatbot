...
module "Chatbot-Telcom-HW-EN-KB" {
  source                    = "github.com/canada-ca-terraform-modules/terraform-azurerm-qna-knowledgebase?ref=20210302.1"
  location                  = local.resource_groups_L2.Telcom_HW_KB.location
  cognitiveServicesLocation = var.cognitiveServicesLocation
  resourceGroupName         = local.resource_groups_L2.Telcom_HW_KB.name
  prefix                    = "${local.prefix}-${var.knowledgebaseList[0].languageCode}-HW"
...
module "Chatbot-Telcom-HW-FR-KB" {
  source                    = "github.com/canada-ca-terraform-modules/terraform-azurerm-qna-knowledgebase?ref=20210302.1"
  location                  = local.resource_groups_L2.Telcom_HW_KB.location
  cognitiveServicesLocation = var.cognitiveServicesLocation
  resourceGroupName         = local.resource_groups_L2.Telcom_HW_KB.name
  prefix                    = "${local.prefix}-${var.knowledgebaseList[1].languageCode}-HW"
